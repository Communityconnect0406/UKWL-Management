...
<body>
      ...
      <div class="card">
        ...
        <div id="tab-infractions" class="tab-content">
          ...
          <div class="input-group">
            ...
            <input id="infraction-staff-id" type="text" />
          </div>
          ...
          <div class="input-group">
            ...
            <input id="infraction-issuer-id" type="text" />
          </div>

          <div class="input-group">
            <label>Reason (required)</label>
            <textarea id="infraction-reason"></textarea>
          </div>

          <div class="input-group">
            <label>Notes (required)</label>
            <textarea id="infraction-notes"></textarea>
          </div>
          ...
          <div class="actions">
            <span id="infraction-status" class="error-text"></span>
        ...
        <div id="tab-promotions" class="tab-content">
          ...
          <div class="input-group">
            <label>Staff member ID (will be pinged)</label>
            <input id="promo-staff-id" type="text" />
          </div>
          ...
          <div class="input-group">
            <label>Issuer ID (will be pinged)</label>
            <input id="promo-issuer-id" type="text" />
          </div>

          <div class="input-group">
            <label>Reason (required)</label>
            <textarea id="promo-reason"></textarea>
          </div>

          <div class="input-group">
            <label>Notes (required)</label>
            <textarea id="promo-notes"></textarea>
          </div>
          ...
          <div class="actions">
            <span id="promo-status" class="error-text"></span>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      // Infractions
      const infStaff = document.getElementById("infraction-staff-id");
      const infType = document.getElementById("infraction-type");
      const infIssuer = document.getElementById("infraction-issuer-id");
      const infReason = document.getElementById("infraction-reason");
      const infNotes = document.getElementById("infraction-notes");
      const infPreview = document.getElementById("infraction-preview");
      const infStatus = document.getElementById("infraction-status");
      const infSend = document.getElementById("infraction-send");

      const infThumbnail = "https://media.discordapp.net/attachments/1489805815559880937/1489996522757029948/Copilot_20260404_151820.png?ex=69d272e5&is=69d12165&hm=bd99e4014ab6013c80ff0f90071604b7967ec4ef2674b04d457c195c69e02425&=&format=webp&quality=lossless&width=1272&height=848";

      function buildInfractionText() {
        const staffPing = infStaff.value ? `<@${infStaff.value}>` : "<@staff>";
        const issuerPing = infIssuer.value ? `<@${infIssuer.value}>` : "<@issuer>";
        const type = infType.value || "Infraction";
        const reason = infReason.value || "(no reason provided)";
        const notes = infNotes.value || "(no notes provided)";
      ...
      function updateInfractionPreview() {
        infPreview.value = buildInfractionText();
      }

      [infStaff, infType, infIssuer, infReason, infNotes].forEach((el) =>
        el.addEventListener("input", updateInfractionPreview)
      );
      updateInfractionPreview();

      infSend.addEventListener("click", async () => {
        const staffId = infStaff.value.trim();
        const issuerId = infIssuer.value.trim();
        const reason = infReason.value.trim();
        const notes = infNotes.value.trim();

        if (!staffId) {
          infStatus.textContent = "Staff member ID is required.";
          return;
        }
        if (!issuerId) {
          infStatus.textContent = "Issuer ID is required.";
          return;
        }
        if (!reason) {
          infStatus.textContent = "Reason is required.";
          return;
        }
        if (!notes) {
          infStatus.textContent = "Notes are required.";
          return;
        }

        infStatus.textContent = "";
        const staffPing = `<@${staffId}>`;
        const issuerPing = `<@${issuerId}>`;
        const type = infType.value;
        const description = buildInfractionText();

        infStatus.textContent = "Sending...";
        infSend.disabled = true;
        ...
        try {
          ...
          if (!res.ok) throw new Error("Request failed");
          infStatus.textContent = "Infraction sent.";
        } catch (err) {
          infStatus.textContent = "Error sending infraction.";
        } finally {
          infSend.disabled = false;
          setTimeout(() => (infStatus.textContent = ""), 4000);
        }
      });

      // Promotions
      const proStaff = document.getElementById("promo-staff-id");
      const proRank = document.getElementById("promo-rank");
      const proIssuer = document.getElementById("promo-issuer-id");
      const proReason = document.getElementById("promo-reason");
      const proNotes = document.getElementById("promo-notes");
      const proPreview = document.getElementById("promo-preview");
      const proStatus = document.getElementById("promo-status");
      const proSend = document.getElementById("promo-send");

      const proThumbnail = "https://media.discordapp.net/attachments/1489806195165237440/1489996980229771395/Copilot_20260404_151347.png?ex=69d27352&is=69d121d2&hm=f12bc3a3419293f5917a508f6965e74f4575af9857a2f2f58eb258f9889aa0f9&=&format=webp&quality=lossless&width=602&height=401";

      function buildPromoText() {
        const staffPing = proStaff.value ? `<@${proStaff.value}>` : "<@staff>";
        const issuerPing = proIssuer.value ? `<@${proIssuer.value}>` : "<@issuer>";
        const rank = proRank.value || "<new rank>";
        const reason = proReason.value || "(no reason provided)";
        const notes = proNotes.value || "(no notes provided)";
      ...
      function updatePromoPreview() {
        proPreview.value = buildPromoText();
      }

      [proStaff, proRank, proIssuer, proReason, proNotes].forEach((el) =>
        el.addEventListener("input", updatePromoPreview)
      );
      updatePromoPreview();

      proSend.addEventListener("click", async () => {
        const staffId = proStaff.value.trim();
        const issuerId = proIssuer.value.trim();
        const rank = proRank.value.trim();
        const reason = proReason.value.trim();
        const notes = proNotes.value.trim();

        if (!staffId) {
          proStatus.textContent = "Staff member ID is required.";
          return;
        }
        if (!rank) {
          proStatus.textContent = "New rank is required.";
          return;
        }
        if (!issuerId) {
          proStatus.textContent = "Issuer ID is required.";
          return;
        }
        if (!reason) {
          proStatus.textContent = "Reason is required.";
          return;
        }
        if (!notes) {
          proStatus.textContent = "Notes are required.";
          return;
        }
        ...
        try {
          ...
          if (!res.ok) throw new Error("Request failed");
          proStatus.textContent = "Promotion sent.";
        } catch (err) {
          proStatus.textContent = "Error sending promotion.";
        } finally {
          proSend.disabled = false;
          setTimeout(() => (proStatus.textContent = ""), 4000);
        }
      });
    });
  </script>
